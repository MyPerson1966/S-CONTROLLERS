...
//			System.out.println(count + "::   " + line);
			    Telescope t = telescopeFromProp(line.split(";"));
			    //System.out.println("Telescope " + t + ";   ");
			    if (t != null) {
				telescopeController.getTelescopeList().add(t);
			    }
			}
		    }
	...
	telescopeController.setFreshUploadComplete(true);;
    }

    private void persistingCollection() {
    ...
    private Telescope telescopeFromProp(String[] prop) {
	if (prop.length == 0) {
	    return null;
	}
	System.out.println("**");
...
//		    System.out.println(k + ":  Mask " + tr);
	...
	if (!testCorrectAngles(ts.getTelescopeMask())) {
	    ts.getTelescopeMask().clear();
	}
	ts.setName(prop[0]);
    ...
    /**
     * The angles of tlk has equal value, result will be incorrect
     *
     * @param ltk has type of TelescopeHorizontMask
     * @return
     */
    private boolean testCorrectAngles(List<TelescopeHorizontMask> ltk) {
	double testval = 0;
	for (int k = 0; k < ltk.size(); k++) {
	    TelescopeHorizontMask tmp = ltk.get(k);
	    testval += tmp.getAngle();
	}
	if (ltk.size() == 0 || testval > 0) {
	    return true;
	} else {
	    return false;
	}
